                    ...
                    <ol class="breadcrumb text-right">
                        <li class="active">
                          <div class="d-flex justify-content-end mb-2">
                            <a href="{{route('album.create')}}" class="btn btn-success ">สร้างอัลบั้ม</a>
                        </div>
                      </li>
                    </ol>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">ตารางอัลบั้ม</strong>
                            </div>
                            <div class="card-body">
                                @if($albums->count()>0)
                                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                                ...
                                                @foreach($albums as $album) <!-- $Categort คือ sql จาก web.php  -->
                                                    <tr>
                                                      <td>{{$album->name}}</td>
                                                      <td>
                                                        <div class="row">
                                                            <form class="imagesindex_form" action="{{route('albumimage.show',$album->id)}}" >
                                                                @csrf
                                                                <input type="hidden" name="_method" value="EDIT">
                                                                <input type="submit" name="" value="แสดง" class="btn btn-outline-success btn-sm">
                                                            </form>
                                                            <form class="images_form" action="{{route('albumimage.edit',$album->id)}}" >
                                                                @csrf
                                                                <input type="hidden" name="_method" value="EDIT">
                                                      ...
                                                      <td>
                                                          <form class="edit_form" action="{{route('album.edit',$album->id)}}" >
                                                              @csrf
                                                              <input type="hidden" name="_method" value="EDIT">
                                                      ...
                                                      <td>
                                                          <form class="delete_form" action="{{route('album.destroy',$album->id)}}" method="post">
                                                              @csrf
