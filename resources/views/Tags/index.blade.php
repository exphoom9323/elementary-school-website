                ...
                <ol class="breadcrumb text-right">
                    <li class="active">
                      <div class="d-flex justify-content-end mb-2">
                        <a href="{{route('tags.create')}}" class="btn btn-success ">สร้างแท็ก</a>
                    </div>
                  </li>
                </ol>
            </div>
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title">ตารางแท็ก</strong>
                        </div>
                        <div class="card-body">
                            @if($tags->count()>0)
                            <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                            ...
                                            @foreach($tags as $tags) <!-- $Categort คือ sql จาก web.php  -->
                                                <tr>
                                                  <td>{{$tags->name}}</td>
                                                  <td>{{$tags->posts->count()}}</td>
                                                  <td>
                                                      <form class="edit_form" action="{{route('tags.edit',$tags->id)}}" >
                                                          @csrf
                                                  ...
                                                  <td>
                                                      <form class="delete_form" action="{{route('tags.destroy',$tags->id)}}" method="post">
                                                          @csrf
