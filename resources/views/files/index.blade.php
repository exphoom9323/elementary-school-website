                    ...
                    <ol class="breadcrumb text-right">
                        <li class="active">
                          <div class="d-flex justify-content-end mb-2">
                            <a href="{{route('files.create')}}" class="btn btn-success ">สร้างไฟล์เอกสาร</a>
                        </div>
                      </li>
                    </ol>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">ตารางไฟล์เอกสาร</strong>
                            </div>
                            <div class="card-body">
                                @if($files->count()>0)
                                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                                ...
                                                @foreach($files as $files) <!-- $Categort คือ sql จาก web.php  -->
                                                    <tr>
                                                      <td>{{$files->title}}</td>
                                                      <td> <a href="{{ route('files.show',$files->id) }}" download>{{$files->name}}</a></td>
                                                      <td>
                                                          <form class="edit_form" action="{{route('files.edit',$files->id)}}" >
                                                              @csrf
                                                      ...
                                                      <td>
                                                          <form class="delete_form" action="{{route('files.destroy',$files->id)}}" method="post">
                                                              @csrf
